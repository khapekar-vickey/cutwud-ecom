jQuery(function($){
    ...
    //function getTemplateByType(data, type){
    function generateShortcodeByType(data, type){
        ...
        if(type == 'tableRow'){
            ...
            //curImgPlaceholder = imageExt == 'spin' ? spinPlaceholder : imPlaceholder;
            let curImgPlaceholder = getPlaceholder(imageSrc);
            let timestamp = data.timestamp != null ? data.timestamp : 'no data';

            //let shName = escape(data.shortcode_options.global_options.shortcodeName) || '';
            let shName = data.shortcode_options.global_options.shortcodeName || '';
            ...
            for(let i=0; i < imagesCount; i++){
                ...
                imagesTemplate += '<img class="sirv-sh-view" data-original="'+ imageSrc +'" src="'+ curImgPlaceholder +'">';
            }
    ...
    function getPlaceholder(itemUrl){
        let imPlaceholder = sirv_ajax_object.assets_path + '/img-plhldr.svg';
        let spinPlaceholder = sirv_ajax_object.assets_path + '/spin-plhldr.svg';
        let videoPlaceholder = sirv_ajax_object.assets_path + '/video-plhldr.svg';
    ...
    function getItemType(itemUrl){
        let videoRegExp = /(mp4|mpg|mpeg|mov|qt|webm|avi|mp2|mpe|mpv|ogg|m4p|m4v|wmv|flw|swf|avchd)/gi;
        let imgRegExp = /(jpg|jpeg|png|gif|svg|webp|bmp)/gi;
    ...
    window['renderShortcodesByType'] = function(data, curPage){
        ...
        if(shortcodes.length > 0){
            ...
            if(appendSelector == '.sirv-choose-wrapper'){
                $(appendSelector).empty();
                $(appendSelector).append('<div class="sirv-no-shortcodes sirv-empty-dir">No shortcodes found. <a href="'+ 'admin.php?page=' + sirv_ajax_object.plugin_path + '/sirv/shortcodes-view.php' +'" target="_blanc">Create shortcode</a></div>');
            }
        }
    ...
    function getSelectedData(elem){
        ...
        let imgParams = '?thumbnail=120&image=true';
        let tmpImages = [];
        let tmpImagesJson = [];
        let tmpCount = count > 4 ? 4 : count;

        for(let i = 0; i < tmpCount; i++){
            tmpImages.push({ 'src': $(images[i]).attr('data-original') + imgParams} );
            tmpImagesJson.push($(images[i]).attr('data-original') + imgParams);
        }
        ...
        function load(imgElem, newImage, src){
            ...
            newImage.onerror = function(){
                if(attemptsToLoadImg > 0 ){
                    setTimeout(function(){load(elem, newImage, src + itemParams);}, 2000);
                    attemptsToLoadImg --;
                }
            }
        }
