jQuery(function($){
    ...
    $(document).ready(function(){
        ...
        function deleteSelectedImages(file=''){
            ...
            // /test44/test%26net/boobs%26guns.m4v
        ...
        function generateGutenbergData(data, id, srImagesAttr={}){
            let images = data.images;
            let count = data.images.length;
            //let shType = window.getShortcodeType(getShortcodeData());
            ...
            if(id){
                for(let i = 0; i < tmpCount; i++){
                    tmpImages.push({ 'src': data.images[i].url + thumbParams} );
                    tmpImagesJson.push(data.images[i].url + thumbParams);
                }
                align = data.align;
            }
            ...
            if(!id && images.length > 0){
                ...
                for(let i = 0; i < images.length; i++){
                    //let link = data.link_image ? data.images[i].url : '';
                    tmpSrImages.push({
                        'src': data.images[i].url + imgParams,
                        'thumb': data.images[i].url + thumbParams,
        ...
        function createGallery(){
            ...
            if($('.selected-miniature-img').length > 0){
                ...
                $('.sirv-add-item-wrapper').bind('click', function(){selectMoreImages(false)});
            }
        ...
        function selectMoreImages(isEditGallery){
            ...
            $('.sirv-add-item-wrapper').unbind('click');
        }
        ...
        window['sirvEditGallery'] = function(id){
            ...
            sendAjaxRequest(ajaxData, processingOverlay='.loading-ajax', showingArea=false, isdebug=false, doneFn=function(response){
                ...
                $('.sirv-add-item-wrapper').bind('click', function(){selectMoreImages(true)});
                $('.insert').bind('click', insert);
                $('.sirv-gallery-type').bind('change', manageOptionsStates);
                $('#gallery-thumbs-position').bind('change', manageThumbPosition);
            });
        } //end sirvEditGallery
