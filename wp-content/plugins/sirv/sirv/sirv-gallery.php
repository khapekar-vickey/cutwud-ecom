...
$html[] = '<div class="sirv-thumbs-box" id="sirv-thumbs-box-'.$this->params['id'].'">';
            if ($this->params['is_gallery'] && !$this->params['apply_zoom'] && count($this->items) > 1) {
                ...
                foreach($this->items as $i=>$item) {
                    ...
                    $html[] = '<li'.(($i==0)?' class="sirv-thumb-selected"':'').'><img title="'.$caption.'" alt="'.$caption.'" data-item-id="'.$i.'" src="'.$item['url'].$profile.((empty($profile))?'?': '&').'thumbnail='.$this->params['thumbnails_height'].((preg_match('/.*\.spin$/is',$item['url']))?'&image=true&scale.option=noup':'').'"/></li>';
                }
                $html[] = '</ul>';
