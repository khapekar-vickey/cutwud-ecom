...
function sirv_gallery($atts){
    ...
    if($js_type == 3){
        wp_enqueue_script('sirv-gallery-mv-viewer', plugins_url('/js/wp-sirv-mv-gallery.js', __FILE__), array('jquery'), false, true);
        require_once 'sirv-gallery-mv.php';
        $gallery = new Sirv_Gallery_MV($options, $images, $captions);
    }else{
        wp_enqueue_script('sirv-gallery-viewer', plugins_url('/js/wp-sirv-gallery.js', __FILE__), array('jquery'), false, true);
