...
class SirvControl extends \Elementor\Base_Data_Control{
	...
	public function enqueue() {
		...
		wp_register_script( 'sirv_logic', SIRV_PLUGIN_URL_PATH . 'sirv/js/wp-sirv.js', array( 'jquery', 'jquery-ui-sortable' ), '1.1.0');
		...
		wp_enqueue_script( 'sirv_modal-logic', SIRV_PLUGIN_URL_PATH . 'sirv/js/wp-sirv-modal.js', array('jquery'), '1.0.0');
		...
		wp_localize_script( 'sirv_modal-logic', 'modal_object', array('media_add_url' =>  SIRV_PLUGIN_URL_PATH . 'sirv/templates/media_add.html', 'login_error_url' => SIRV_PLUGIN_URL_PATH . 'sirv/templates/login_error.html', 'featured_image_url' => SIRV_PLUGIN_URL_PATH . 'sirv/templates/featured_image.html', 'isNotEmptySirvOptions' => $isNotEmptySirvOptions, 'sirv_cdn_url' => get_option('SIRV_CDN_URL')));
		wp_enqueue_script('sirv-shortcodes-page', SIRV_PLUGIN_URL_PATH . 'sirv/js/wp-sirv-shortcodes-page.js', array( 'jquery'), '1.0.0');

		wp_enqueue_script( 'sirv_control_manager', plugins_url('/assets/js/sirvControlManager.js', __FILE__), array('jquery'), '1.0.0', true);

		//wp_register_style('wp-standard-styles', get_admin_url() . "load-styles.php?c=0&dir=ltr&load[]=dashicons,admin-bar,buttons,media-views,common,forms,admin-menu,dashboard,list-tables,edit,revisions,media,themes,about,nav-menu&load[]=s,wp-pointer,widgets,site-icon,l10n,wp-auth-check");
		...
		//wp_enqueue_script('sirv-js', 'https://scripts.sirv.com/sirv.js', array(), false);
	}
